...
class EncoderDecoder(BaseSegmentor):
    ...
    def encode_decode(self, img, img_metas):
        ...
        import numpy as np
#         print(out[1][:,1:].cpu().numpy().shape)
#         np.save('/data02/liliulei/embedding2/'+img_metas[0]['ori_filename'][:-4],out[1].cpu().numpy())
...
#                 x_swap = output.clone()
#                 for (ii, jj) in zip([14, 16, 18, 15, 17, 19], [15, 17, 19, 14, 16, 18]):
#                     x_swap[:,ii, :, :]=output[:, jj, :, :]
#                 output = x_swap
